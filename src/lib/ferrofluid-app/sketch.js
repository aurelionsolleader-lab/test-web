...
import envMap01Url from '../../assets/env-map-01.jpg';
...
export class Sketch {
    ...
    constructor(canvasElm, onInit = null, onEntryAnimationDone = null, isDev = false) {
    ...
    #updatePointer() {
        this.pointerLerp[0] += (this.pointer[0] - this.pointerLerp[0]) / 1.2;
        this.pointerLerp[1] += (this.pointer[1] - this.pointerLerp[1]) / 1.2;
    ...
    #animate(deltaTime) {
        ...
        if (this.isEntryAnimationDone) {
            let targetZoom = 0.5;
            if (this.audioControl && this.audioControl.isInitialized) {
                const val = this.audioControl.getValue();
                if (val > -1) {
                    targetZoom = 0.5 + Math.max(0, val) * 0.5;
                }
            }
